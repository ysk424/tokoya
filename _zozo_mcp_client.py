...
This must not run on Blender's main thread: ZOZO MCP queues every mutation back
to that thread, so a synchronous request from a Blender operator would wait on
itself.  Tokoya launches this script with Blender's bundled Python and reads the
single JSON status line it prints on stdout.
"""
        ...
        reply = self._post(
            {
                ...
                "params": {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "tokoya", "version": "0.7.0"},
                },
            },
            timeout=5.0,
        )
...
def _configure(config: dict) -> dict:
    ...
    try:
        ...
        owned_names = {config["rod_group"], config["body_group"]}
        for group in groups:
            if group.get("name") in owned_names and group.get("uuid"):
                client.call_tool("delete_group", {"group_uuid": group["uuid"]})

        rod = client.call_tool(
            "create_group", {"name": config["rod_group"], "type": "ROD"}
        )
        rod_uuid = str(rod["group_uuid"])
        client.call_tool(
            "add_objects_to_group",
            {"group_uuid": rod_uuid, "object_names": [config["rod_object"]]},
        )
        client.call_tool(
            "set_group_material_properties",
            {"group_uuid": rod_uuid, "properties": config["rod_properties"]},
        )
